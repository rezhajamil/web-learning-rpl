    ...
    <div class="flex flex-col flex-1 w-full">
        ...
        <main class="h-full overflow-y-auto">
            <div class="container grid px-6 mx-auto my-3 gap-y-4">
                ...
                <form method="post" action="{{ route('admin.lesson.update',$lesson->id) }}" class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <trix-editor input="desc">{!! old('desc',$lesson->desc) !!}</trix-editor>
                    @error('desc')
                    ...
                    <span class="text-neutral-900 file-preview">{{ 'File Chosen : '.$lesson->file_name }}</span>
                    @error('file')
                    <span class="text-xs text-red-600 dark:text-red-400">
                        {{ $message }}
                    </span>
